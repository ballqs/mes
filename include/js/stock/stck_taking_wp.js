...
$(function () {
    ...
    pop_prt_nbr_cd_wp.AddTargetColumnsOnPopupClose(["prt_nbr_cd","prt_nbr_nm","spec","account_type","base_unit"], ["prt_nbr_cd","prt_nbr_nm","spec","account_type","stck_unit"]);
    ...
    //팝업등록
    pageManager.gridManager.AddPopUpOption(grid01.name, "prt_nbr_cd", pop_prt_nbr_cd_wp.PopupName);
    ...
    pageManager.gridManager.SetPkFields(grid01.name, ["fact_cd", "taking_ymd", "prt_nbr_cd", "ship_cd"]);
    pageManager.gridManager.SetCompulsoryFields(grid01.name, ["whs_cd","taking_qty","taking_gbn_cd"]);
    pageManager.gridManager.SetReadonlyFields(grid01.name, ["prt_nbr_nm","spec","account_type","whs_nm","stck_unit","biz_cd","biz_nm","ship_nm"]);

    //버튼에 대한 Ajax 경로
    pageManager.gridManager.SetAjaxUrl(grid01.name, Const.MesButton.Search, "/ajax/stock/stck_taking/get/stck_taking_wp");
    pageManager.gridManager.SetAjaxUrl(grid01.name, Const.MesButton.DeleteRow, "/ajax/stock/stck_taking/delete/stck_taking_wp");
    pageManager.gridManager.SetAjaxUrl(grid01.name, Const.MesButton.Save, "/ajax/stock/stck_taking/save/stck_taking_wp");
                ...
                for(let i=0; i<checknum; i++){
                    for(let j=0; j<param.length; j++){
                        if((checkdata[i]["fact_cd"] === param[j]["fact_cd"]) &&
                            (checkdata[i]["ship_cd"] === param[j]["ship_cd"]) &&
            ...
            if(grid_records[idx].w2ui !== undefined && grid_records[idx].w2ui.changes !== undefined){
                if(grid_records[idx]["confrm_yn"] === true){
                    ...
                    data[i] = {
                        ...
                        "lotno" : grid_records[idx]['ship_cd'],
                        "confrm_yn" : "Y",
                    };
                    i++;
                }
    ...
    $.ajax({ // 처음 웹페이지 로딩시 표시하기 위해
        ...
        success: function (data) {
            if(data.data[0]["cd_set1"] == 'Y'){
                grid01.columns[11] = { field: 'loc_cd', caption: '위치코드', size: '100px', sortable: true, resizable: true };
                grid01.columns[12] = { field: 'loc_nm', caption: '위치', size: '150px', sortable: true, resizable: true};
            }else{
