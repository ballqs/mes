...
$(function () {
    ...
    let grid01 = {
        ...
        columns: [
            ...
            { field: 'mach_nm', caption: '설비', size: '100px', sortable: true, resizable: true, editable: { type: 'text', maxLength: 50}},
            //select
            { field: 'mach_type', caption: '설비타입', size: '100px', sortable: true, resizable: true, editable: { type: 'select' }},
            ...
            { field: 'cavity', caption: 'CAVITY', size: '100px', sortable: true, resizable: true, editable: { type: 'int' }, attr:'align=right'},
    ...
    pageManager.gridManager.SetPkFields(grid01.name, ["fact_cd", "mach_cd"]);
    //pageManager.gridManager.SetReadonlyFields(grid01.name,  ["mach_nm"]);
    pageManager.gridManager.SetCompulsoryFields(grid01.name, ["mach_nm","mach_type","cavity"]);

    //버튼에 대한 url 등록
    pageManager.gridManager.SetAjaxUrl(grid01.name, Const.MesButton.Search, "/ajax/base/product/get/eqpmt_mgt");
    pageManager.gridManager.SetAjaxUrl(grid01.name, Const.MesButton.DeleteRow, "/ajax/base/product/delete/eqpmt_mgt");
    pageManager.gridManager.SetAjaxUrl(grid01.name, Const.MesButton.Save, "/ajax/base/product/save/eqpmt_mgt");

    //콤보박스 렌더 정보 등록
    pageManager.gridManager.AddSelectBoxInfo(grid01.name,"fact_cd", common.code, {up_cd: "fact_cd"}, "cd", "cd_nm");
    pageManager.gridManager.AddSelectBoxInfo(grid01.name,"mach_type", common.code, {up_cd: "mach_type"}, "cd", "cd_nm");

    //체크박스 T/F 정보 등록
    pageManager.gridManager.AddCheckRenderOption(grid01.name,"perfrm_colct_yn", "Y", "N");
    pageManager.gridManager.AddCheckRenderOption(grid01.name,"fail_inst_yn", "Y", "N");
    ...
    let target_name = ["mach_cd","mach_nm"];    //popup창을 뛰우고 거기서 클릭한 데이터를 넘겨 받기 위한 타겟
