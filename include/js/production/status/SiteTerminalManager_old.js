function tableManager(table_selector){
    ...
    // }
    let checkbox_name = '';
    let checkbox_column_no = 0;
    let checkbox_info = {};
    let selected_rows = [];
    let records = [];
    function set_hidden_cols(hidden_col_names_arr){
        if(hidden_col_names_arr){
            this.hidden_cols = hidden_col_names_arr;
        }
    }
    function set_no_data_msg(msg){
        this.no_data_msg = msg;
    }
    function set_checkbox(checkbox_name, checkbox_column_no){
        this.checkbox_name = checkbox_name;
        this.checkbox_column_no = checkbox_column_no;
    }
    function add_checkbox(checkbox_name, checkbox_column_no, checkbox_value = '', checkbox_key = ''){
    ...
    function draw_tbody(url, param, column_arr, key_column = column_arr[0], callbackFnc = {}){
    ...
    function trOnClickTypeInputHidden(target_input_selector, class_name = 'Ton', callback){
    ...
    function trOnClickTypeCheckbox(class_name = 'on', callback){
        $(`${this.table_selector} > tbody`).on("click", "tr", function(){
            console.log(this);
            // if(callback.pre !== undefined) callback.pre();
            // if(this.dataset.key !== undefined) {
    ...
    function registerSelectTypeCheckbox(checkbox_name){

    }
    function onChangeSelectTypeCheckbox(){
