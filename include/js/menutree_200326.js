...
function get_pgm_id_list(pgm_id, pgm_info){
  $.ajax({
      ...
      dataType:'json',
      success:function(data){
        pgm_info.pgm_id_list = new Array();
...
function draw_main_menu(menu_set, target_el, class_name){ // 메뉴 array, 타겟 요소, ul 클래스명)
  ...
  for (let i = 0; i < menu_set.length; i++) {
    ...
    if (menu_set[i].pgm_gbm == 'M') {
      click_func = "onclick='menu_tgl(this);'";
    ...
    html += "<li" + is_curr_page + "><a "+click_func+" id='" + menu_set[i].pgm_id + "_" + menu_set[i].up_pgm_id + "_" + menu_set[i].pgm_gbm + "'>" + menu_set[i].pgm_nm + drop_down_arrow + "</a>";
    depth = menu_set[i].depth;
  }
  html += "</li>";
  $(target_el).html(html);
