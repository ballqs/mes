...
$(function () {
    ...
    let grid01 = {
        ...
        columns: [
            ...
            { field: 'fact_cd', caption: '공장', size: '80px', sortable: true, hidden:true},
        ...
        fncPreSavePage : function(){
            ...
            for(let key in w2ui.grid02.records){
                if(w2ui.grid02.records[key].stck_qty !== undefined){
                    if(Number(w2ui.grid02.records[key].stck_qty) < Number(w2ui.grid02.records[key].w2ui.changes.out_qty)){
                        $(".loadingW").css("display", "none");
                        common.mes_alert({msg : (Number(key)+1)+"열에 출고 수량이 현재 재고보다 많습니다."});
                        return result;
                    }else if(Number(w2ui.grid02.records[key].w2ui.changes.out_qty) <= 0 || String(w2ui.grid02.records[key].w2ui.changes.out_qty).trim() == ''){
                        $(".loadingW").css("display", "none");
