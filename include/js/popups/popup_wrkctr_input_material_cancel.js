...
$(function(){
    let popup_id = "popup_wrkctr_input_material_cancel";
    let form_id = "popup_wrkctr_input_material_cancel_frm";
    let form_name = "popup_wrkctr_input_material_cancel_frm";

    let param = FormHelper.SerializeForm(form_id);
    let get_url = "/ajax/production/status/get/curr_material_list_in_man_whs_loc";
    let get_url_prtnbr = "/ajax/production/status/get/curr_material_list_in_man_whs_loc_prtnbr";
    let key_col = 'lotno';
    ...
    let target_input_selector = `form[name='popup_sel_wrkctr_orderno_frm'] input[name='hidden_wrk_ordr_no']`;

    let table_lot = new tableManager("#table_can_lot");
    table_lot.add_checkbox("chk_list", 0, 'sprt_nbr', 'lotno');

    table_lot.set_hidden_cols(['stck_whs_cd', 'stck_loc_cd', 'stck_unit']);
    table_lot.set_class(['','','text_r','','','text_r','','']);
    table_lot.trOnClickTypeCheckbox('on');

    let table_prt = new tableManager("#table_can_prt");
    ...
    let draw_tbl = function(search_type){
        if (search_type == 'lot') {
            ...
            table_lot.draw_tbody(get_url, {param : param}, table_arr_lot, key_col);
        ...
        if (this.dataset.name == "init") {
            ...
            w2confirm('진행하시겠습니까?')
              .yes( () => {
                  ...
                  $(`#${form_id} #table_can_lot > tbody > tr`).each(function () {
                  ...
                  $.ajax({
                      url: "/ajax/production/status/delete/insert_material",
                      type: "post",
                      data: {
                          param: param,
                      ...
                      success: function (res) {
                          ...
                          if (res.result) {
                              draw_tbl('lot');
                              draw_tbl('prt');
                              $(`#table_can_prt`).css("display", "none");
                              let d = new Date();
