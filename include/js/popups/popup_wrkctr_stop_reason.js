...
$(function(){
    let popup_id = "popup_wrkctr_stop_reason";
    let form_id = "popup_wrkctr_stop_reason_frm";
    let form_name = "popup_wrkctr_stop_reason_frm";
    ...
    $(`#${popup_id} #${stop_gbn_title_id}`).on('click', 'div', function(){
        ...
        //리스트 뿌리기
        popup_wrkctr_stop_reason.searchPage(search_url);
    ...
    $(`#${form_id} .opBtn`).on("click", function(){
        ...
        if (this.dataset.name == "init") {
            ...
            $.ajax({
                url: "/ajax/production/status/save/stop_reason",
                type: "post",
                data: {
                    param:param,
            ...
            $(`#${popup_id}`).css("display", "none");
            $(`#popup_wrkctr_stop_reason .btn`).each(function(){
                if($(this).hasClass("on")){
                    $(this).removeClass("on");
                }
            });
...
class popup_wrkctr_stop_reason{
    ...
    static searchPage(url, callback = { pre: null, post: null}, msg = true){
        ...
        let param = FormHelper.SerializeForm("popup_wrkctr_stop_reason_frm");
        $.ajax({
            url: url,
            type: "get",
            data: {param: param},
            ...
            success: function (data) {
                let frm_id = 'popup_wrkctr_stop_reason_frm';
                console.log(data);
                popup_wrkctr_stop_reason.drawCont(data, `#${frm_id} .btnCont`);
                $(`#${frm_id} input[name='page']`).val(data.page);
                if (msg) { popup_wrkctr_stop_reason.success_msg(data.msg); }
